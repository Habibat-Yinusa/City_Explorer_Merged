...
const BusinessServices = () => {
  ...
  const { businessData } = location.state as businessProps;
  ...
  if (businessData) {
    setBusinessInfo((prev) => {businessData});
  } else if (data) {
    setBusinessInfo((prev) => {data});
  } else {
    navigate("/explore");
  }
      ...
      >
        ...
        >
          <CenteredBox sx={{ width: "7em", marginTop: "2em" }}>
            <img src={businessData.logo} alt="" style={{ width: "100%" }} />
          </CenteredBox>
          <Typography
            variant="h3"
            sx={{
